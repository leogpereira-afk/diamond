...
Pacote local, sem publicação. Base: e60eff39577e3c30dacf63f63ca42ba32f552727. Identidade visual preservada.
...
Não há reconciliação automática de registros históricos. Numeração repetida, relações por nome, indicadores de conversão e reabertura de histórico por perfil foram identificados na nova auditoria e ficam para correção específica. O pedido não é uma transação conjunta com alteração simultânea de status da unidade.
...
Publicar as duas funções `dmd-api` e `dmd-p` com o adaptador compartilhado atualizado, além de `app.js`, `store.js`, `p.html` e `sw.js` (cache v15). Não há migração de banco neste pacote. A mudança do servidor é compatível com clientes autenticados existentes.
