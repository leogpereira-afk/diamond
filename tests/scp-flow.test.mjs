...
test('envio SCP aponta para a proposta salva e mostra o mesmo preco negociado',async()=>{
 ...
 vm.runInContext(code.slice(0,code.lastIndexOf("  window.addEventListener('hashchange'"))+`\n gerarPdfScp = async () => ({doc:{output:()=> 'data:application/pdf;base64,JVBERi0='}});crmRegistrar=async()=>({id:'lead-1'});_scp={unidadeId:'u-1',cliente:'Cliente Teste',tel:'11999999999',descPct:'10',p12:true,nParc:'12',p2412:false,nFix:'12',nCor:'12',corr:0};vScp();globalThis.submit=()=>document.querySelector('#scp-whats').onclick();})();`,c);
 await c.submit();assert.ok(saved);assert.ok(meta);assert.equal(saved.neg,270000);assert.equal(meta.valor,270000);assert.equal(meta.propostaId,saved.id);assert.equal(meta.leadId,'lead-1');assert.equal(saved.leadId,'lead-1');
});
